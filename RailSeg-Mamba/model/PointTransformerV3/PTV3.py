...
def offset2batch(offset):
    ...
    #print("bincount",bincount) #bincount tensor([32648], device='cuda:0')
    return torch.arange(
...
class SerializedPooling(PointModule):
    ...
    def forward(self, point: Point):
        ...
        #print(point.feat[indices])
        #print(point.coord[indices])
        # collect information
        point_dict = Dict(
...
class PointTransformerV3(PointModule):
    def __init__(
        ...
        self.class_num = 11
        self.seghead = nn.Conv1d(64,self.class_num,1,1,0)
    def forward(self, data_dict):
        ...
        return self.seghead(point["feat"].unsqueeze(0).permute(0,2,1))

if __name__ == "__main__":
    device = torch.device("cuda:0")
    point = torch.randn((32648,3),dtype=torch.float32).to(device)
    #grid = torch.randint(10000,(32648,3),dtype=torch.int64).to(device)
    grid = point
    offset = torch.tensor([32648],dtype=torch.int64).to(device)
    grid_size = torch.tensor([0.05],dtype=torch.float32).to(device)
    data = {
        "feat":point,
        "coord":grid,
        "grid_size":grid_size,
    ...
    result = model(data)
    print(result["feat"].shape)

    """
    Number of parameter:  46.1550M
    """